...
export function Categories({ navigation, currentTheme }: Props) {
  ...
  useEffect(() => {
    ...
    for (let cat of categories) {
      budget += cat.budget;
      spent += cat.expenditure;
    }
    setTotal({ budget, spent });
  }, [categories]);

  if (isLoading) {
    return (
      <>
        <Card style={[styles.cardStyle, { marginBottom: 10, justifyContent: 'center' }]}>
          <DefaultView style={{ width: WIDTH - 40 }}>
            <LoadingSkeleton style={{ flexDirection: 'row', marginTop: 0, justifyContent: 'space-between' }} itemStyle={{ marginBottom: 10 }}>
              <DefaultView style={{ width: 100, height: 15 }}/>
              <DefaultView style={{ width: 100, height: 15 }}/>
            </LoadingSkeleton>
            <LoadingSkeleton style={{ flexDirection: 'row', justifyContent: 'space-between' }} itemStyle={{ marginBottom: 10 }}>
              <DefaultView style={{ width: 100, height: 15 }}/>
              <DefaultView style={{ width: 100, height: 15 }}/>
            </LoadingSkeleton>
          </DefaultView>
  ...
  return (
    <>
      <Card style={styles.cardStyle}>
        ...
        <View style={styles.cardItem}>
          <Text style={styles.cardSubHeader}>Total Spent</Text>
          <Text style={styles.cardSubHeader}>{total.spent}</Text>
        </View>
      </Card>
      <Card style={[styles.cardStyle, { marginBottom: 10, minHeight: 240 }]}>
        ...
        <HorizontalCardList
          ...
          renderItem={({ item: { name, budget, expenditure } }) => {
            ...
            const percentSpent = expenditure/budget;
            const expenseMultiple = percentSpent < 0 ? 0 : percentSpent;
